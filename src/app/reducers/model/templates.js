...
export default function (state = Immutable.Map(), action = {}) {
    ...
    switch (action.type) {
        ...
        case ActionTypes.GET_TENANT_TEMPLATE_LIST:
            newState = state.merge(Immutable.fromJS(action.normalizedData.entities.templates));
            return newState.equals(state) ? state : newState;

        case ActionTypes.UPSERT_TEMPLATE:
            newState = state.set(action.template.id.toString(), Immutable.fromJS(action.template));
            return newState.equals(state) ? state : newState;
