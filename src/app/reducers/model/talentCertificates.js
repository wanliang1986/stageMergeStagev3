...
export default function (state = Immutable.OrderedMap(), action = {}) {
    ...
    switch (action.type) {
        case ActionTypes.RECEIVE_TALENT:
            newState = state.merge(Immutable.fromJS(action.normalizedData.entities.talentCertificates));
            return newState.equals(state) ? state : newState;

        case ActionTypes.EDIT_TALENT_CERTIFICATE:
        case ActionTypes.ADD_TALENT_CERTIFICATE:
            return state.set(action.certificate.id.toString(), Immutable.fromJS(action.certificate));
