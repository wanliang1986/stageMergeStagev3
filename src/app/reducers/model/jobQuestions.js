...
export default function (state = Immutable.OrderedMap(), action = {}) {
    ...
    switch (action.type) {
        case ActionTypes.RECEIVE_JOB:
            newState = state.merge(Immutable.fromJS(action.normalizedData.entities.jobQuestions));
            return newState.equals(state) ? state : newState;

        case ActionTypes.ADD_JOB_INTERVIEW_QUESTION:
        case ActionTypes.EDIT_JOB_INTERVIEW_QUESTION:
            return state.set(action.jobQuestion.id.toString(), Immutable.Map(action.jobQuestion));
