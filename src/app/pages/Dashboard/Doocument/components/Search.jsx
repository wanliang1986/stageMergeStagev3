...
class Search extends Component {
  ...
  handleSearch = (e) => {
    ...
    dispatch(newRegularSearch(type, newArr));
    ...
    if (packageName && type == 'packageName') {
      // let packageNamedarr = [];
      // arr.forEach((item3) => {
      //   item3.values.forEach((item4) => {
      //     packageNamedarr.push(String(item4.value));
      //   });
      // });
    ...
    if (documentName && type == 'documentName') {
    ...
    if (inputDataList.length === 0) {
      inputData.forEach((item) => {
        if (type === item.key) {
          dispatch(newDocumentSearch(interfaceData));
        }
      });
    } else {
      dispatch(newDocumentSearch(params));
    }
...
const mapStateToProps = (state) => {
  return {
    inputData: state.controller.documentView.toJS().searchDataList || [],
