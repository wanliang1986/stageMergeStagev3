...
import PackageSaveDocumentDialog from './components/PackageSaveDocumentDialog';
...
import {
  ...
  newPackageSearch,
  newPackDeleteSearch,
  newInterfacePackageDeleteSearch,
  newPackSaveFiltersName,
...
class PackFilterSearch extends Component {
  constructor(props) {
    super(props);
    this.state = {
      Newcolumns: [],
  ...
  handleDelete = (data, e) => {
    ...
    let interfaceDataList = loadsh.cloneDeep(this.props.packInterfaceDataList);
    ...
    if (interfaceDataList.search[0].condition.length === 0) {
      interfaceDataList.timezone = new Date(
        moment().startOf('day').toDate()
      ).getTime();
    }
    this.props.dispatch(newPackageSearch(interfaceDataList));
    this.props.dispatch(newPackDeleteSearch(showList));
    this.props.dispatch(newInterfacePackageDeleteSearch(interfaceDataList));
  };
  // props改变
  ...
  handleClearAllAdvanced = () => {
    this.props.dispatch({
      type: ActionTypes.PACKAGE_CLEAR_ALL,
    });
    this.props.dispatch({
      type: ActionTypes.DOCUMENT_LODING,
      payload: true,
    });
    let interFaceList = loadsh.cloneDeep(this.props.packInterfaceDataList);
    interFaceList.search[0].condition = [];
    interFaceList.timezone = new Date(
      moment().startOf('day').toDate()
    ).getTime();
    this.props.dispatch({
      type: ActionTypes.PACKAGE_INTERFACE_CLEAR_ALL,
      payload: interFaceList,
    });
    this.props.dispatch(newPackageSearch(interFaceList));
  };
  // Save Filters事件
  handleSaveFilter = (e) => {
  ...
  handleSaveFilters = (e) => {
    ...
    let obj = {
      ...
      // searchContent: JSON.stringify(this.props.packInterfaceDataList),
      searchName: JSON.stringify(this.state.FilterNameValue.substring(0, 20)),
      searchGroup: JSON.stringify(this.props.packInterfaceDataList),
    ...
    this.props.dispatch(newPackSaveFiltersName(a));
    this.handleClose();
  };
  render() {
    ...
    return (
      <div style={{ ...styles.search }}>
        ...
        <DeleteFilter
          ...
          newDeleteData={this.state.SavedFilterArr}
        />
      </div>
    );
  }
...
const mapStateToProps = (state) => {
  ...
  let packInterfaceDataList =
    state.controller.documentView.toJS().packInterfaceDataList || [];
  let objArr = state.controller.documentView.toJS().saveFiltersObj || [];
  return {
    searchShow,
    packInterfaceDataList,
    objArr,
  };
};

export default connect(mapStateToProps)(withStyles(styles)(PackFilterSearch));
