...
class DocumentFrom extends Component {
  ...
  getColumn = () => {
    getPackageDefaultColumns().then(({ response }) => {
  ...
  onChangeSort = (columnKey, sortDir) => {
    ...
    dispatch(newPackageSearch(interList));
  };
  //保存用户columns设置
  saveColumns = (data) => {
  ...
  render() {
    const { classes, loading, packageViewFromData, count } = this.props;
    const { size, headerList, page } = this.state;
    console.log('headerList', headerList);
    if (loading && packageViewFromData) {
      return (
        <div
            ...
            >
              ...
              >
                <AgGridReact
                  ...
                  rowData={packageViewFromData}
                  // onGridReady={this.onGridReady}
                  // rowData={DocumentViewData}
...
const mapStateToProps = (state) => {
  console.log(state.controller.documentView.toJS());
  let loading = state.controller.documentView.toJS().loading;
  let packageViewFromData =
    state.controller.documentView.toJS().packageViewFromData || [];
  ...
  let pages = state.controller.documentView.toJS().packInterfaceDataList
    ? state.controller.documentView.toJS().packInterfaceDataList.pageNumber * 1
    : null;

  let sizes =
    state.controller.documentView.toJS().packInterfaceDataList?.pageSize;
  return {
    packageViewFromData,
    loading,
    interfaceDataList,
