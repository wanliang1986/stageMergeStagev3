...
class JobBasicForm extends React.PureComponent {
  ...
  handleContactNameChange = (clientcontactname) => {
    console.log(clientcontactname);
    this.setState({ clientcontactname });
    const clientContact = this.state.clientContactNameList.find(
      (c) => c.id === clientcontactname
    );
    this.setState({
      clientcontactUsername: clientContact ? clientContact.name : '',
    });
  };
  ...
  // 这是循环的location，UI显示还有点问题，数据是对的，还需要组装
            ...
            <div className="row expanded">
              <div className=" columns">
                <FormInput
                  ...
                  errorMessage={
                    errorMessage ? errorMessage.get('salary') : null
                  }
                  onBlur={() => {
                    if (removeErrorMsgHandler) removeErrorMsgHandler('salary');
                  }}
                />
              </div>
              <div>
