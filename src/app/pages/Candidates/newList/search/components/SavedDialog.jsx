...
function SavedDialog({ show, close, t }) {
  ...
  const handleChange = (e) => {
    ...
    clearTimeout(timer);
    let time = setTimeout(() => {
      getFilterSearchCandidate(e.target.value).then((res) => {
        console.log(res);
        if (res) {
          let { response } = res;
  ...
  const handleSearch = (e, item) => {
    ...
    if (item.searchType == 'BASE') {
      ...
      dispatch(saveAdvancedFilterCandidate(JSON.parse(item.searchGroup)));
    }
    handleClose();
  };
  ...
  const showChipStr = (str, type) => {
    if (type == 'BASE') {
      let { arr, strShow, strBox } = candidateFilterSearch(JSON.parse(str));
