...
import {
  commonPoolSaveAdvancedFilterCandidate,
  upDateStopFlag,
...
export default function AdvancedDialog({ show, close }) {
  ...
  // Search事件
  const handleSearch = () => {
    let { data, isOk } = candidateIsRequiredAdvanced(lodash.cloneDeep(value));

    if (isOk) {
      setArrs(data);
      dispatch(commonPoolSaveAdvancedFilterCandidate(data));
      close();
    } else {
