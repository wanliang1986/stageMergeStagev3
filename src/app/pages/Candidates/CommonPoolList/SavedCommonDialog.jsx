...
import {
  getCommonPoolSavedSearchData,
  setInFilter,
  saveAdvancedFilterCommonPool,
...
function SavedDialog({ show, close, t }) {
  ...
  const handleChange = (e) => {
    ...
    clearTimeout(timer);
    let time = setTimeout(() => {
      getFilterSearchCommonPool(e.target.value).then((res) => {
        console.log(res);
        if (res) {
          let { response } = res;
  ...
  const handleSearch = (e, item) => {
    ...
    if (item.searchType == 'BASE') {
      ...
      dispatch(getCommonPoolSavedSearchData());
    } else if (item.searchType == 'ADVANCED') {
      dispatch(saveAdvancedFilterCommonPool(JSON.parse(item.searchGroup)));
    }
    handleClose();
  };
  ...
  const handleDeleteOk = () => {
    deleteFilterCandidate(deleteId).then((res) => {
  ...
  const showChipStr = (str, type) => {
    if (type == 'BASE') {
      let { arr, strShow, strBox } = commonPoolFilterSearch(JSON.parse(str));
      let res = strShow.split(',');
      res.pop();
      return res;
    } else {
      let { showStr } = candidateGetAdvincedFilter(JSON.parse(str));
      let arr = [];
      showStr.forEach((item) => {
        arr = arr.concat(item[1].split(','), 'or');
      });
