...
class AssignmentPage extends Component {
  constructor(props) {
    ...
    this.state = {
      // assignment: null,
      status: this.props.assignment ? this.props.assignment.status : 'PENDING',
      bill: this.props.assignment,
      isExempt: false,
      payRate: null,
      errorMessage: Immutable.Map(),
      createType:
        this.props.assignment && this.props.assignment.type
  ...
  componentDidMount() {
    const { currentStart, dispatch, assignmentId } = this.props;
    dispatch(getClientContactList(currentStart.get('companyId')));
    dispatch(getAssignmentDetail(assignmentId));
    dispatch(getAssignmentCurrent(currentStart.get('id')));
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.assignment) {
      this.setState({
        startDate:
          nextProps.assignment.startDate &&
          moment(nextProps.assignment.startDate),
        endDate:
          nextProps.assignment.endDate && moment(nextProps.assignment.endDate),
  ...
  save = () => {
    const { currentStart, t, dispatch, assignmentId } = this.props;
    const { status, startDate, endDate, isExempt, createType } = this.state;
    ...
    let obj = {
      ...
      billInfo: {
        contactId: billForm.BillingContact.value,
        isExcept: isExempt,
    ...
    dispatch(updateAssignment(obj))
      .then((res) => {
        ...
        if (res) {
          ...
          dispatch(showSuccessMessage(`UpDate Assignment Success`));
        }
      })
      .catch((error) => {
  ...
  delete = () => {
    ...
    const { dispatch, assignmentId, currentStart } = this.props;
    dispatch(deleteAssignmentList(assignmentId)).then((res) => {
      if (res === 200) {
        dispatch(getAssignmentCurrent(currentStart.get('id'))).then((res) => {
          console.log(res);
          this.setState(
            {
              deleteStatus: false,
            ...
            () => {
              this.props.history.push({
                pathname: `/candidates/assignment/${res.id}`,
                state: { pageType: 'default' },
              });
              dispatch(getAssignmentDetail(res.id));
            }
          );
        });
      }
  ...
  timeJudge = (date) => {
    ...
    let endDate = moment(date)
      ...
      .valueOf();
    if (newDate >= endDate && assignment.status !== 'PENDING') {
      return false;
    }
    return true;
  };
  ...
  render() {
    ...
    let editType = this.timeJudge(assignment.endDate);
    ...
    return (
      <div className={classes.root}>
          ...
          <div className={classes.assignmentOption}>
            <div className={classes.title}>
              <Typography variant="h6">
                {assignment && assignment.type === 'EXTENSION'
                  ? `Assignment(Extension ${
                      assignment.order > 1 ? assignment.order : ''
                    })`
                  : assignment && assignment.type === 'RATE_CHANGE'
              ...
              <Grid item xs={4}>
                ...
                <Button
                  ...
                  disabled={
                    ...
                    !this.timeJudge(endDate) ||
                    assignment.isClockIn === true ||
                    assignment.isSubmitExpenses === true
                  }
                >
                  Delete Assignment
                </Button>
          ...
          <Grid container spacing={3}>
            <Grid item xs={6}>
              <BillCard
                ...
                createdTime={assignment && assignment.createdTime}
                startId={assignment && assignment.startId}
                talentId={assignment && assignment.talentId}
                billInfo={assignment && assignment.billInfo}
                timeSheet={assignment && assignment.timeSheet}
                workingLocation={assignment && assignment.workLocation}
                userInfo={
                  assignment
                    ? assignment.userInfo
                    : currentStart.get('startCommissions')
                }
                pageType={location.state.pageType}
                clientList={clientList}
                contactsList={contactsList}
                isClockIn={assignment ? assignment.isClockIn : false}
                isSubmitExpenses={
                  assignment ? assignment.isSubmitExpenses : false
                }
            ...
            <Grid item xs={6}>
              <PayCard
                ...
                createdTime={assignment && assignment.createdTime}
                startId={
                  assignment ? assignment.startId : currentStart.get('id')
                }
                talentId={
                  assignment
                    ? assignment.talentId
                    : currentStart.get('tenantId')
                }
        ...
        <Dialog aria-labelledby="simple-dialog-title" open={openDeleteDialog}>
