...
import { getAgingReport, getAgingReportFilter } from '../../../../apn-sdk/report';
...
class AgingReport extends React.PureComponent {
    ...
    fetchData = (from, to, company, recruiter_id, blockTimerPromise = Promise.resolve()) => {
        ...
        this.candidateTask.promise
            .then((data) => {
                ...
                if (columnKey) {
                    ...
                    filteredIndex = sortList(preIndex, dataList, columnKey, sortDir).push(preIndex.size)
                }

                blockTimerPromise
                    .then(() => this.setState({ loading: false, dataList, filteredIndex }))
            })
            .catch((reason) => {
    ...
    fetchFilters = (from, to) => {
        ...
        getAgingReportFilter({ from_date, to_date })
            .then(({ response }) => {
                // console.log('[????]', response);
                let responseCompanyList = response.companyList ? response.companyList : []
                const companyList = [{ value: '', label: 'All' }].concat(responseCompanyList.map(ele => {
                    return {
                        value: ele,
                        label: ele
                    };
                ...
                const lastModifiedUserList = [{
                    value: '',
                    label: 'All'
                }].concat(response.recruiterList.map(ele => {
                    const { id, firstName, lastName, username } = ele;
                    return {
                        value: id,
                        label: (firstName && lastName) ? firstName.trim() + ' ' + lastName.trim() : username
                    };
                }));
    ...
    handleDateRangeChange = ({ startDate: from, endDate: to }) => {
        ...
        if (from && to) {
            ...
            this.fetchData(from, to, selectedCompany, selectedLastModifiedUser, this._blockTimer());
            this.fetchFilters(from, to);
        }
    ...
    handleClickActivityCount = ({ activityId, company, recruiter, activityStatus }) => {
        // console.log('[[activity]]', activityId, company, recruiter, activityStatus);
        const title = 'Aging Report';
        const { from, to } = this.state;
        const range = `presented between ${from.format('MM/DD/YYYY')} and ${to.format('MM/DD/YYYY')}`;
        this.setState({ openActivityDetail: { activityId, title, range, activityStatus } })
    };
    ...
    render() {
        ...
        const {
            dataList, from, to,
            selectedCompany, companyList, lastModifiedUserList, selectedLastModifiedUser,
            loading, openActivityDetail, filteredIndex, colSortDirs
        } = this.state;
        ...
        return (
            ...
            <Paper className={clsx('flex-child-auto flex-container flex-dir-column', classes.root)}>
                <div>
                    <Typography variant='h5' className='item-padding'>
                        {t('message:Aging Report')}
                    </Typography>

                    <div className={clsx('horizontal-layout align-bottom', classes.actions)}>
                        ...
                        <div>
                            <div style={{ minWidth: 168, height: 53 }}>
                                <FormReactSelectContainer
                                    label={t('field:Lastmodified User')}
                                >
                                    <Select
                ...
                <div className={clsx('flex-child-auto', classes.contentContainer)}>
                    {loading ?
                        <div className={clsx('flex-container flex-dir-column', classes.contentMask)}>
                            <Loading />
                        </div>
                        : <ReportTableSummary
...
export default withTranslation(['tab', 'action', 'field', 'message'])(connect()(withStyles(styles)(AgingReport)))
