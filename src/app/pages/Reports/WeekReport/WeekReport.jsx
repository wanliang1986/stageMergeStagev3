...
import { getWeekReport, getWeekReportExcel } from '../../../../apn-sdk/index';
import { sourceTypes } from '../../../constants/formOptions';
...
class WeekReport extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    this.fetchData(from, to, selectedType);
  }
  ...
  fetchData = (
    ...
    this.candidateTask = makeCancelable(
      getWeekReport({ from_date, to_date, selectedType })
    );
  ...
  downloadData = () => {
    ...
    const from_date = moment(from).hours(0).utc().format();
    const to_date = moment(to).hours(0).utc().format();
    getWeekReportExcel({ from_date, to_date, selectedType })
      .catch(() =>
        dispatch({
  ...
  handleDateRangeChange = ({ startDate: from, endDate: to }) => {
    ...
    if (from && to) {
      ...
      this.fetchData(from, to, selectedType, this._blockTimer());
    }
  };
          ...
          >
            ...
            <div>
              <div style={{ minWidth: 168, height: 53 }}>
                <FormReactSelectContainer label={t('field:Source Type')}>
                  <Select
                    value={selectedType}
                    options={typeOptions}
                    simpleValue
