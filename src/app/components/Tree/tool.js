...
export const parentChain = (TreeDataMap, parentNode, config, checkedList) => {
  if (parentNode) {
    const checkStatus = childCheckedStatus(
      parentNode.children,
      TreeDataMap,
      config
    );
    ...
    if ('undefined' !== typeof parentNode.parentVal) {
      parentChain(
        TreeDataMap,
        TreeDataMap[parentNode.parentVal],
        config,
        checkedList
      );
    }
  ...
  idList.forEach((item) => {
    ...
    if (
      ...
      (!isEmptyArray(children) &&
        filterListCheckChildren(children, treeDataMap, val))
    ) {
      // console.log(value)
...
export const checkedCheckedList = (treeDataMap, checkedList, checkbox) => {
  ...
  checkedList.forEach((value) => {
    ...
    checkbox.parentChain &&
      parentVal &&
      _parentChain(treeDataMap, treeDataMap[parentVal], checkbox, checkedList);
  });
};
...
export const _parentChain = (treeDataMap, parentNode) => {
  if (parentNode) {
    const isHalf = parentNode.children.some((item) => {
      ...
      if (
        currentNode.checkStatus.checked ||
        currentNode.checkStatus.halfChecked
      ) {
        return true;
      }
      return false;
