...
class TreeSelect extends Component {
  constructor(props) {
    ...
    this.onClickRowExpand = this.onClickRowExpand.bind(this);
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (nextProps.data !== prevState.treeData) {
      const initCheckedListVal = defaultConfig.checkbox.enable
        ? defaultConfig.checkbox.initCheckedList.map((item) => [
            item.toString(),
            item.toString(),
          ])
        : [];
      const initCheckedList = new Map(initCheckedListVal);
      const { map, idList, renderIdList, checkedList } = generateTreeDataMap(
        {},
        nextProps.data,
        defaultConfig,
        initCheckedList,
  ...
  onClickRowExpand(item, e) {
    ...
    if (isExpand) {
      ...
      if (!isEmptyArray(item.children)) {
        ...
        let _children = item.children.concat([]);
        // 如果搜索框有值，要过滤一次
        if (searchVal) {
          ...
          _children = arr.filter((item) => {
            ...
            if (
              ...
              (!isEmptyArray(children) &&
                filterListCheckChildren(children, treeDataMap, searchVal))
            ) {
              treeDataMap[value] = {
                ...treeDataMap[value],
                isExpand: true,
  ...
  componentDidMount() {
    const { onChecked, selectedVal } = this.props;
    const { treeDataMap, checkedList, updateListState, checkbox } = this.state;
    if (selectedVal) {
      for (let key in treeDataMap) {
        let keyVal = key;
        selectedVal.forEach((item, index) => {
          if (item.label === keyVal) {
            // treeDataMap[keyVal].checkStatus.checked = true
            this.onChecked(treeDataMap[keyVal]);
          }
        });
      }
    }
  ...
  onChecked(item, e) {
    ...
    // 处理复选框联动逻辑
    e &&
      checkbox.parentChain &&
      parentChain(
        _treeDataMap,
        _treeDataMap[item.parentVal],
        checkbox,
        _checkedList
      );
    e &&
      checkbox.childrenChain &&
      childrenChain(_treeDataMap, item.children, _checked, _checkedList);
    ...
    this.setState(
      ...
      () => {
        console.log(this.state.treeDataMap);
        e &&
          onChecked &&
          onChecked(
            Array.from(this.state.checkedList.keys()),
            this.state.treeDataMap,
            e
          );
      }
    );
  ...
  treeNodeRender({
    ...
    const checkedClassName = item.checkStatus.checked
      ...
      : '';
    const halfCheckedClassName =
      !item.checkStatus.checked && item.checkStatus.halfChecked
        ...
        >
          ...
          >
            {!isEmptyArray(item.children) && (
              <i
                className={`${item.isExpand && prefixClassName + '-expand'}`}
              ></i>
            )}
          </div>
          {_checkbox.enable && !disabled && (
            ...
            >
              <span
                className={`${prefixClassName}-checkbox-inner ${
                  disabled ? 'disabled' : ''
                }`}
              ></span>
            </div>
          ...
          <div
            ...
            title={item.title}
            className={`${prefixClassName}-title ${
              isSelectVal ? 'active' : ''
            }`}
          >
            {customTitleRender ? customTitleRender(item) : item.label}
          </div>
  ...
  render() {
    ...
    const prefixClassName = defaultProps.prefixClassName;
    const _className = `${prefixClassName}-TreeSelect ${
      wrapperClassName || ''
    }`;
    if (loading) {
      return (
