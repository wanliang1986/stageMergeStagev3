...
const CreditsCell = ({ rowIndex, data, col, ...props }) => {
  const id = data.getIn([rowIndex, 'id']);
  const activated = data.getIn([rowIndex, 'activated']);
  if (id) {
    let credit =
      data && data.getIn([rowIndex, 'monthlyCredit'])
        ? data.getIn([rowIndex, 'monthlyCredit'])
        : 0;
    let usedCredit =
      data && data.getIn([rowIndex, 'usedMonthlyCredit'])
        ? data.getIn([rowIndex, 'usedMonthlyCredit'])
        : 0;
    let creditVal = usedCredit + '/' + credit;
    if (activated) {
      return (
        <Cell {...props}>
          <div
...
const BulkCreditsCell = ({ rowIndex, data, col, ...props }) => {
  const id = data.getIn([rowIndex, 'id']);
  const activated = data.getIn([rowIndex, 'activated']);
  if (id) {
    let credit =
      data && data.getIn([rowIndex, 'bulkCredit'])
        ? data.getIn([rowIndex, 'bulkCredit'])
        : 0;
    let usedCredit =
      data && data.getIn([rowIndex, 'usedBulkCredit'])
        ? data.getIn([rowIndex, 'usedBulkCredit'])
        : 0;
    let creditVal = usedCredit + '/' + credit;
    if (activated) {
      return (
...
const TableCell = ({
  ...
  switch (type) {
    ...
    case 'credits':
      return <CreditsCell {...props} />;
    case 'bulkCredits':
      return <BulkCreditsCell {...props} />;
    default:
          ...
          <React.Fragment>
            ...
            >
              ...
              {onDelete && (
                <Column
                  cell={({ rowIndex, ...props }) => {
                    const id = userList.getIn([rowIndex, 'id']);
                    return (
                      <Cell {...props}>
                        <IconButton
                          onClick={() => onDelete(id)}
