...
export default function MultipleSelect(props) {
  ...
  useEffect(() => {
    // getSelectList(props.defaultSelected[0], props.defaultSelectedLabel);
    getDefaultSelectList();
  }, [props.defaultSelected]);

  const getDefaultSelectList = () => {
    if (props.defaultSelected.length === props.options.length) {
      let list = lodash.cloneDeep(props.defaultSelected);
      list.push('All');
      setSelectedList(list);
      setSelected(['All']);
    } else {
      setSelectedList(props.defaultSelected);
      let defaultSelected = FilterData(props.options, props.defaultSelected);
  ...
  const FilterData = (a, b) => {
    ...
    for (var i = 0; i < a.length; i++) {
      if (b.indexOf(a[i].value) != -1) {
        result.push(a[i].label);
      }
    }
    return result;
  };
    ...
    if (_index === -1) {
      ...
      if (newSelectList.length === 1) {
        ...
        if (val !== 'All') {
          ...
          newSelectList = props.defaultSelect;
          newSelected = props.defaultSelectedLabel;
        }
      }
    }
